...
[디자인 방향]
- 16:9 비율, 한국어 발표 자료, 총 9장으로 구성해 주세요.
...
- 다이어그램은 슬라이드 한 장에 핵심 블록 5~7개 이내로 제한하고, 발표 화면에서도 글자가 읽히도록 크게 배치합니다.
...
- 과목명과 6조 팀원 역할을 간단히 표시합니다.
- 스마트폰 앱 화면 목업 또는 앱 아이콘을 중심 비주얼로 배치합니다.

Slide 2. 문제 정의와 프로젝트 목표
- 기존 알람은 기기마다 설정과 제어 상태가 분리된다는 문제를 보여줍니다.
- 목표: Android 기본 알람 기능을 구현하고, 로그인한 사용자가 여러 Android 기기에서 같은 알람 변경 상태를 공유할 수 있도록 하는 것입니다.
- '문제 -> 해결 방향' 2단 비교 인포그래픽으로 표현합니다.

Slide 3. 주요 기능과 실제 UI
- Android MVP 기능: 알람 생성/수정/삭제, 활성화/비활성화, 요일 반복, 소리/진동, 스누즈, 잠금화면 울림 화면, 로그인/그룹 동기화.
- 홈 화면, 알람 편집 화면, 알람 울림 화면 스크린샷 3개를 배치할 영역을 만듭니다.
- 기능 설명은 스크린샷 주변의 짧은 라벨로만 표시합니다.

Slide 4. 전체 시스템 구조도
- 제목: Firebase 기반 동기화 알람 구조
- 다음 구성 요소를 블록 다이어그램으로 표현합니다.
  사용자 -> Flutter Android 앱 -> Cloud Firestore -> Cloud Functions -> FCM -> 다른 Android 앱
- Flutter Android 앱과 다른 Android 앱 내부에 각각 'Android 로컬 알람 예약 / 정각 울림' 블록을 표시합니다.
- Firebase Authentication은 Flutter Android 앱과 연결된 보조 인증 블록으로 표시합니다.
- 화살표 라벨:
  1. 알람 저장/수정
  2. 변경 감지
  3. 변경 데이터 메시지
  4. 로컬 예약 갱신
- 강조 문구: 'FCM은 변경 전달, 실제 울림은 Android 로컬 예약'

Slide 5. 알람 생성 및 동기화 클래스 흐름
- UML 클래스 상속도가 아니라 가독성 높은 시퀀스 다이어그램으로 표현합니다.
- 참여 객체: 사용자, AlarmEditorSheet(UI), AlarmListController, AlarmRepository, Cloud Firestore, Cloud Functions/FCM, 다른 Android 기기
- 흐름:
  사용자 저장 -> UI 입력 전달 -> Controller 상태 처리 -> Repository 저장 -> Firestore 반영 -> Functions 변경 감지 -> FCM 전달 -> 다른 기기 로컬 예약 갱신
- 로컬 모드에서는 Repository가 SharedPreferences에 저장하고, 로그인/그룹 동기화 모드에서는 Firestore를 사용한다는 작은 주석을 추가합니다.

Slide 6. 알람 울림, 해제, 스누즈 기능 흐름
- 세로형 기능 플로우 차트로 표현합니다.
- 흐름:
  Android 로컬 예약 시간 도달 -> 잠금화면 알람 화면/소리/진동 -> Dismiss 또는 Snooze 선택
  - Dismiss: 현재 울림 종료
  - Snooze: 설정된 시간 뒤 로컬 재예약
  - 로그인 및 그룹 사용 시: 제어 상태를 Firebase를 통해 다른 기기에 전달
- '오프라인이어도 이미 로컬에 예약된 알람은 울릴 수 있음'을 보조 문구로 표시합니다.

Slide 7. 데이터 구조와 보안
- 핵심 Firestore 컬렉션만 단순한 트리 또는 데이터 카드로 표현합니다.
  users/{uid}
...
Slide 8. 시연 순서
- 발표 중 실제로 따라갈 4단계 시연 흐름을 가로 스텝으로 표현합니다.
  1. 알람 생성 및 세부 설정
  2. 지정 시간에 잠금화면 알람 울림 확인
  3. 로그인/그룹 연결 후 두 Android 기기 간 알람 변경 동기화 확인
  4. Dismiss 또는 Snooze 제어 동기화 확인
- 동기화 시연이 네트워크 상태에 따라 실패할 수 있으므로 화면 캡처 또는 짧은 녹화 영상을 백업 자료로 배치할 공간을 남겨 주세요.

Slide 9. 구현 결과, 확장 계획, Q&A
- 구현 결과: Android 알람 기본 기능, Firebase 계정/그룹 기반 동기화, 로컬 예약 기반 울림.
- 확장 계획: Web/Windows/Linux 지원, 알람 기록, 벨소리 선택 등은 '향후 구현'으로 표시합니다.
- 하단에 Q&A와 감사 문구를 배치합니다.

[출력 요구]
- 각 슬라이드에는 제목, 핵심 한 문장, 사용할 시각 자료 유형을 명확히 구성해 주세요.
- 구조도와 흐름도에는 아이콘보다 읽기 쉬운 박스와 화살표를 우선 사용하세요.
- 시연 및 구현 완료 여부를 사실 이상으로 과장하지 마세요.
```
